...
Description:           This code is modified from Arun M Saranathan
...
class minMapTools():
    ...
    def create_Maps4CRISMImages_Cosine(self, model, mica_data, imgName, endMem_Name='', scaleFlag=False,
        ...
        mica_data = torch.tensor(mica_data).double()
        mica_dataPreds_l2 = np.asarray(model(mica_data))
        ...
        arrImg = torch.tensor(arrImg).double()
        imgPreds = np.asarray(model(arrImg))
