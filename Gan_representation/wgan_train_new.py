...
def main():
    ...
    for epoch in range(epoch_num):
        for step, data in enumerate(dl):
            ...
            # gradient penalty
            eps = torch.Tensor(b_size, 1).uniform_(0, 1)
            # print(eps.size())
            # print(data.size())
            # print(fake.size())
            data = data.to(device)
            ...
            if step % 5 == 0:
                print('[%d/%d][%d/%d]\tLoss_D: %.4f\tLoss_G: %.4f'
                      % (epoch, epoch_num, step, len(dl), loss_D.item(), loss_G.item()))
        ...
        with torch.no_grad():
            ...
            plt.close()
        # Save outputs using different noise vectors
