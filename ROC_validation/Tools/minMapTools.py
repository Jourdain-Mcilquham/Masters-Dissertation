...
class minMapTools():
    ...
    def create_Maps4CRISMImages_Cosine_test(self, mica_data, imgName):
        mica_dataPreds_l2 = np.asarray(mica_data)
        ...
        img_flat = [item for sublist in cubeOrig for item in sublist]

        mica_dataPreds_l2 = np.nan_to_num(mica_dataPreds_l2)
        img_flat = np.nan_to_num(img_flat)
        dist = np.squeeze(cosineDist(mica_dataPreds_l2, img_flat))

        simMap = np.zeros((700, 700, mica_dataPreds_l2.shape[0]))
        for em in range(mica_dataPreds_l2.shape[0]):
            distMap = np.squeeze(dist[em, :])
        ...
        'Save this map in the same folder as the input image'
        envi.save_image('tester.hdr', simMap, dtype=np.float32, force=True, interleave='bil')
    ...
    '------------------------------------------------------------------------------------------------------------------'
    def create_Maps4CRISMImages_GuessMap(self, imgName, colMat, thresholds):
        """
