...
class crismProcessing_parallel():
    ...
    def clean_ratio(self, imgName, d):
        """
        Function divides each pixel by a denominator
        :param imgName: The name of the image
        :return: The name of the image
        """
        img = envi.open(imgName)
        header = envi.read_envi_header(imgName)
        cube = img.load().copy()
        [rows, cols, bands] = cube.shape

        d = cube[d[0], d[1]]
        final = np.empty((rows, cols, bands), dtype=np.float32)
        for i in tqdm(range(rows)):
            for j in range(cols):
                final[i,j] = cube[i,j]/d

        
        outFileName = f'{imgName[:-4]}_clean.hdr'
        envi.save_image(outFileName, final, dtype='single',
                        force=True, interleave='bil', metadata=header)

        
    def fnCRISM_contRem_nb(self, imgName, filter=True, filterSize=11):
        """
    ...
    def fnCRISM_createCRImg(self, imgName, bgFileName):
        ...
        'Now load the image'
        bgImg = envi.open(bgHdrName)
        header = envi.read_envi_header(bgHdrName)
        bgCube = bgImg.load().copy()
        bgCube = np.flip(bgCube, axis=(0,1))
