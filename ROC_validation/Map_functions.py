...
from Tools.models import Discriminator
...
class Map_functions:
    ...
    def normalise_img(self, f_name):
        ...
        envi.save_image('normalised_classification.hdr', img_normalised, dtype='float32', force=True, interleave='bil')
        return 'normalised_classification.hdr'



    def create_map(self, f_name, i_name, tester = False):
        endMem_Name = ''
        ...
        spectra_name = self.normalise_img(spectra_name) if normalise_flag else spectra_name
        ...
        simScoreMapName = minMapTools(0, 240).create_Maps4CRISMImages_Cosine(discriminator, exemplars,
                                                                                spectra_name, endMem_Name)
        if(tester):
            minMapTools(0, 240).create_Maps4CRISMImages_Cosine_test(exemplars, spectra_name)

        '--------------------------------------------------------------------------------------------------'
        'STEP-8: IDENTIFY SIGNIFICANT PIXEL SPECTRA & THRESHOLD SIMILARITY MAPS TO MINIMIZE FALSE POSITIVES'
        '--------------------------------------------------------------------------------------------------'

        maskName = minMapTools(0, 240).create_Mask4CRISMImages(spectra_name)
